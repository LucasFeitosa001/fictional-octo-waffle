...
// An empty VITE_API_ORIGIN means "same origin as the current host". Better Auth's
// createAuthClient rejects a relative baseURL ("Invalid base URL"), so we resolve
// the empty origin to the absolute window.location.origin at runtime instead of
// emitting a relative "/api/v1/auth".
const API_ORIGIN =
  CONFIGURED_ORIGIN ||
