...
type SoldFilter = 'all' | PackageStatus;
const SOLD_FILTERS: { id: SoldFilter; label: string }[] = [
  { id: 'all', label: 'Todos' },
...
export function PacotesPage() {
  ...
  const allSoldRows = sold.data?.data ?? [];
  const templateRows = templates.data ?? [];

  const soldRows = useMemo(() => {
    const term = search.trim().toLowerCase();
    return allSoldRows.filter((p) => {
      const st = p.status === 'active' && p.isExpired ? 'expired' : p.status;
  ...
  function exportSoldCsv() {
    downloadCsv(
      `pacotes-vendidos-${new Date().toISOString().slice(0, 10)}`,
      [
        { header: 'Cliente', value: (p: CustomerPackage) => p.customer?.name ?? '' },
        { header: 'Número', value: (p: CustomerPackage) => p.number },
        { header: 'Modelo', value: (p: CustomerPackage) => p.template?.name ?? '' },
        { header: 'Valor', value: (p: CustomerPackage) => p.price },
        { header: 'Validade', value: (p: CustomerPackage) => formatDate(p.expiresAt) },
        { header: 'Sessões usadas', value: (p: CustomerPackage) => p.sessionsUsed },
        { header: 'Sessões totais', value: (p: CustomerPackage) => p.sessionsTotal },
        { header: 'Sessões restantes', value: (p: CustomerPackage) => p.sessionsRemaining },
        {
          header: 'Status',
          value: (p: CustomerPackage) =>
            STATUS_LABELS[p.status === 'active' && p.isExpired ? 'expired' : p.status],
        },
      ],
      soldRows,
    );
                    ...
                    <li key={i.serviceId} className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setItems((prev) => prev.filter((x) => x.serviceId !== i.serviceId))}
                        >
                          Remover
                        </Button>
                      </div>
